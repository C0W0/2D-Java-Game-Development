...
public class Slider extends UIObject{
    ...
    public void onMouseRelease(MouseEvent e) {
        ...
        if(hovering){
            if(horizontal){
                value = (int)((e.getX()-x)/width*(max-min)+min+0.5);
                if(value < min){
                    value = min;
                } else if(value > max){
                    value = max;
                ...
                value = (int)((y+height-e.getY())/height*(max-min)+min+0.5);
                if(value < min){
                    value = min;
                } else if(value > max){
                    value = max;
    ...
    public void render(Graphics graphics){
        ...
        if(horizontal){
            ...
            graphics.drawImage(slider,(int)((float)(value-min)/(max-min)*width+x),(int)y,height,height,null);
        } else {
            for(int i = 0; i <= max/tickSpacing; i++){
                graphics.drawImage(tickMark,(int)x,(int)((float)i*tickSpacing/max*height+y),width,width/2,null);
            }
            graphics.drawImage(slider,(int)x,(int)(y+height-(float)(value-min)/(max-min)*height),width,width,null);
        }
